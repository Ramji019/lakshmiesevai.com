                ...
                <div class="col-xl">
                    <div class="card mb-4">
                        <div class="card-body">
                            ...
                            @endif
                            <form action="{{ url('/update_pmkissan') }}" id="formAccountSettings" method="POST"
                                ...
                                <div class="mb-3 col-md-6">
                                    @if (Auth::user()->id != $apply_user_id)
                                        @if ($services->land_document != '')
                                            <label>Land Document (All Pages)</label>
                                            <br><a target="_blank"
                                                href="{{ URL::to('/') }}/upload/services/landdocument/{{ $services->land_document }}"
                                                class="btn btn-primary me-2">View</a><br>
                                        @endif
                                    @else
                                        <label>Land Document (All Pages)</label>
                                        <input @if ($services->land_document == '') required @endif class="form-control"
                                            type="file" accept="image/jpeg, image/png" name="land_document">
                                        @if ($services->land_document != '')
                                            <a target="_blank"
                                                href="{{ URL::to('/') }}/upload/services/landdocument/{{ $services->land_document }}">Download</a><br>
                                        @endif
                                    @endif
                                </div>
                                @if (Auth::user()->id != $apply_user_id)
                                    <div class="mb-3 col-md-6">
                                        ...
                                        <select class="form-control" name="status" id="service_status">
                                            ...
                                            <option @if ($services->status == 'Resubmit') selected @endif value="Resubmit">
                                                Resubmit</option>
                                            <option @if ($services->status == 'Processing') selected @endif value="Processing">
                                                Processing</option>
                                    ...
                                    <div class="mb-3 col-md-6" id="applicationnohide" style="display :none;">
                                        ...
                                        <input value="{{ $services->application_no }}" class="form-control" type="text"
                                            maxlength="20" name="application_no" id="application_no" />
                                    </div>
